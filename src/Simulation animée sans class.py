...
] = "C:\\Users\\Portable Guillaume\\Documents\\ffmpeg-20160318-git-a7b8a6e-win64-static\\bin\\ffmpeg.exe"
...
def doubler(pos, i, vdv, vdr1, vdv1, vdv0, vdr0, nf):
    ...
    if (
        ...
        if (
            ecart0dr > pos[vdr0, 2] * T or vdr0 == i
        ):  # distance correspondant à 2 sec a la vitesse
            if (
                ecart0dv > pos[vdv0, 2] * T and pos[vdv0, 2] + 1 >= pos[i, 2]
            ) or vdv0 == i:
                res = -1
                pos[i, 4] = 15 / dt
    return res
...
def etape(veh, dt, t, freq, nf, v, posmin):
    ...
    for j in range(0, len(veh)):
        ...
        if double != 0 and posmin[veh[j, 1]][1] == j:
            posmin[veh[j, 1]] = [veh[vdv, 0], vdv]
        elif posmin[veh[j, 1]][1] == j:
    ...
    if time % freq == freq - 1:  # Test si on peut ajouter un véhicule
        maxdesmin = [
            ...
            posmin[0, 1],
            0,
        ]  # position de la voiture, indice de la voiture, file où elle se trouve
        ...
        if maxdesmin[1] >= len(veh):
            ...
            posmin[0] = [veh[len(veh) - 1, 0], len(veh) - 1]
            maxdesmin = [posmin[0][0], posmin[0, 1], 0]
        if veh[maxdesmin[1], 0] > -(long - d):
            ...
            veh = np.vstack(
                (
                    veh,
                    np.array(
                        [-n * (L + d), maxdesmin[2], vit, attribueVitesse(1, v), 0]
                    ),
                )
            )
...
def vitesses_moyennes(pos, nf):
    ...
    vit_moy = [0] * (
        nf + 1
    )  # liste des vitesses moyenne par file, le dernier représente la vitesse moyenne globale
...
def etat_initial(v, nf):
    ...
    init_state = np.zeros(
        (n, 5)
    )  # Matrice [N*5] comme décrite dans l'entête de la class Vehicules
    init_state[:, 0] = z
...
def simulation_animee(freq, nf, v):
    ...
    ax = fig.add_subplot(111, autoscale_on=False, xlim=(-long - 1, xm), ylim=(-0.3, nf))
    ...
    ani = animation.FuncAnimation(
        fig, animate, frames=500, interval=50, blit=True, init_func=init
    )
    ...
    plt.text(
        0.63, 0.87, chaine, transform=ax.transAxes, bbox=dict(ec="black", fc="white")
    )
    plt.yticks(
        [i for i in range(nf)], ["Voie " + str(i + 1) for i in range(nf)], size="medium"
    )
    plt.show()
...
def principal(nb, freq, nf, v):
    ...
    for i in range(nb):
        ...
        vit, flux, nbVhpKm = vitesses_moyennes(
            veh, nf
        )  # list [vitesse moy par file + vitesse moy global], flux, veh.km-1
        if (
            i % (1 / dt) == 0 and i > 1000
        ):  # 1 fois par seconde et une fois le régime permanent établit
            flxMoy += flux
            nbVhpKmMoy += nbVhpKm
            vitMoy += vit[-1]
            compteur += 1
        ...
        if i % 327 == 0:
            ...
            k += 1
            ax = fig.add_subplot(
                111, autoscale_on=False, xlim=(-long - 1, xm), ylim=(-0.3, nf)
            )
            chaine = (
                "Flux entrant : "
                + str(int(3600 / (freq * dt)))
            ...
            plt.yticks(
                [i for i in range(nf)],
                ["Voie " + str(i + 1) for i in range(nf)],
                size="medium",
            )
    ...
    if compteur == 0 or nbiter == 0:
        print(
            "Pas assez ditération. Merci de rentrer une valeur de nb supérieure à 2000"
        )
        return None
    else:
...
def graph_voie(nb, v):
    lflx = [50 * i for i in range(1, 30)]  # flux en veh.h-1
    # lflx=[360,720]
    ...
    for nf in lnf:
        ...
        for freq in lfreq:
            ...
            a, b, c, d = principal(
                nb, freq, nf, v
            )  # a flux moy, b flux global, c vit, d veh.km-1
            print((a + b) / 2, c)
...
def etape2(veh, dt, t, freq, nf, v, posmin):
    """Renvoie la liste des véhicules actualisée et le nombre de voitures qui ont été supprimées"""
    global time, fig
    time += 1
    ...
    if time == 13 * 20:
        vit = 89 / 3.6
        veh = np.vstack((veh, np.array([-n * (L + d), 0, vit, vit, 0])))
        posmin[1] = [veh[-1, 0], -150]

    # veh = np.delete(veh,indadelete,axis=0)

    return veh, len(indadelete), indadelete


def etat_initial2(v, nf):
    z = np.arange(
        0, -(n - 1) * (L + d) - 1, -(L + d)
    )  # Positions initiales des voitures
    init_state = np.zeros(
...
def simulation_animee2(freq, nf, v):
    ...
    def animate(i):
        global pos, veh, voit
        veh, plus, indadel = etape2(veh, dt, i, freq, nf, v, posmin)
        pos = ax.scatter(veh[:, 0], veh[:, 1], c=actu_couleurs(veh, v), marker="s")
        vitmoy, flx, osef = vitesses_moyennes(veh, nf)
        # anotationy.set_yticks([i for i in range(nf)], ['Voie '+str(i+1)+'\nVit moy : '+str(int(vitmoy[i])) for i in range(nf)], size='medium')
        stat.set_text(
...
def principal2(nb, freq, nf, v):
    ...
    for i in range(nb):
        veh, voitplus, indadel = etape2(veh, dt, i, freq, nf, v, posmin)
        vit, flux, nbVhpKm = vitesses_moyennes(
            veh, nf
        )  # list [vitesse moy par file + vitesse moy global], flux, veh.km-1
        if (
        ...
        if i % 40==0:
            ...
            for j in range(len(veh)):
                if not j in indadel:
                    plt.text(veh[j,0],veh[j,1]-0.08,str(j+1), size='small')
    ...
    if compteur == 0 or nbiter == 0:
        ...
        vitMoy = vitMoy / compteur
        vph = 3600 * nbvoittot / ((nbiter) * dt)
        return flxMoy, vph, vitMoy, nbVhpKmMoy, veh[0, 0]
